...
/* Keep the generated form source self-checking: this correction closes the
 * serious-misconduct renderer's HTML string before the following function. */
const SAFE_FORM = FORM
  .replace("</div>')}function updateState", "</div>';}function updateState")
  .replace('.review{background:#fbf9f5;border:1px solid var(--line);padding:14px;border-radius:10px;white-space:pre-wrap}', '.review{background:#fbf9f5;border:1px solid var(--line);padding:14px;border-radius:10px}.review-row{display:grid;grid-template-columns:minmax(170px,32%) 1fr;gap:12px;border-bottom:1px solid #e8e1d6;padding:9px 0;overflow-wrap:anywhere}.review-row:last-child{border-bottom:0}.review-label{color:var(--muted);font-weight:700;text-transform:capitalize}.review-value{white-space:pre-wrap}@media(max-width:650px){.review-row{grid-template-columns:1fr;gap:2px}}')
  .replace('if(current===5)document.getElementById(\'review\').textContent=JSON.stringify(serialize(),null,2);', 'if(current===5)renderReview();')
  .replace('function validStep(){', "function renderReview(){var data=serialize(),labels={manager_name:'Submitting manager',manager_email:'Manager email',employee_name:'Employee',employee_id:'Employee ID',store_code:'Store',position:'Position',state:'State',hire_date:'Original hire date',last_day_worked:'Last day worked',separation_date:'Effective separation date',separation_code:'Separation code',rehire_code:'Rehire recommendation',authority_code:'Authority',protected_flags:'Protected-circumstance review',access_items:'Access held'};var review=document.getElementById('review');review.innerHTML='';Object.keys(data).filter(function(key){return key!=='request_token'&&data[key]!==''&&data[key]!==null&&data[key]!==undefined}).forEach(function(key){var line=document.createElement('div'),label=document.createElement('div'),value=document.createElement('div');line.className='review-row';label.className='review-label';value.className='review-value';label.textContent=labels[key]||key.replace(/_/g,' ');value.textContent=Array.isArray(data[key])?data[key].join(', '):String(data[key]);line.appendChild(label);line.appendChild(value);review.appendChild(line)})}function validStep(){")

module.exports = { FORM: SAFE_FORM, submit, STORES, REASONS, REHIRE, validate };
